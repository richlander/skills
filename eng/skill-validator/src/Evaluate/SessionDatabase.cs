...
public sealed class SessionDatabase : IDisposable
{
    ...
    /// 4 = added <c>sessions.expected_skills</c> and <c>sessions.eval_mode</c>.
    /// </summary>
    ...
    private void Initialize()
    {
        ...
        EnsureSessionsExpectedSkillsColumn();
        EnsureSessionsEvalModeColumn();
    ...
    private void EnsureSessionsExpectedSkillsColumn()
    {
        if (HasColumn("sessions", "expected_skills"))
            return;

        using var cmd = _connection.CreateCommand();
        cmd.CommandText = "ALTER TABLE sessions ADD COLUMN expected_skills TEXT";
        cmd.ExecuteNonQuery();
    }

    private void EnsureSessionsEvalModeColumn()
    {
        if (HasColumn("sessions", "eval_mode"))
            return;

        using var cmd = _connection.CreateCommand();
        cmd.CommandText = "ALTER TABLE sessions ADD COLUMN eval_mode TEXT";
        cmd.ExecuteNonQuery();
    }
    ...
    public void RegisterSession(string sessionId, string skillName, string skillPath,
        ...
        IReadOnlyList<string>? expectedSkills = null, string? evalMode = null)
    {
        var expectedSkillsJson = expectedSkills is null
            ? null
            : JsonSerializer.Serialize(expectedSkills.ToArray(), SkillValidatorJsonContext.Default.StringArray);
        lock (_lock)
        {
            using var cmd = _connection.CreateCommand();
            cmd.CommandText = """
                INSERT INTO sessions (id, skill_name, skill_path, scenario_name, run_index, role, model, config_dir, work_dir, prompt, skill_sha, rubric, baseline_key, expected_skills, eval_mode, status, started_at)
                VALUES ($id, $skill_name, $skill_path, $scenario_name, $run_index, $role, $model, $config_dir, $work_dir, $prompt, $skill_sha, $rubric, $baseline_key, $expected_skills, $eval_mode, 'running', $started_at)
                """;
            ...
            cmd.Parameters.AddWithValue("$expected_skills", (object?)expectedSkillsJson ?? DBNull.Value);
            cmd.Parameters.AddWithValue("$eval_mode", (object?)evalMode ?? DBNull.Value);
    ...
    private List<SessionRecord> GetSessions(string whereClause)
    {
        ...
        while (reader.Read())
        {
            results.Add(new SessionRecord(
                ...
                ExpectedSkillsJson: reader.IsDBNull(17) ? null : reader.GetString(17),
                EvalMode: reader.IsDBNull(18) ? null : reader.GetString(18)));
        }
        return results;
    }
...
public sealed record SessionRecord(
    ...
    string? ExpectedSkillsJson = null,
